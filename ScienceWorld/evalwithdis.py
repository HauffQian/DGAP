...
from utils import load_model, findValidActionNew, load_variation, get_model_output, get_model_detee, get_model_roberta, findValidActionWithSystem2, getFilteredValidActions, sbert_search, clean_look, is_action_failed
from utils import try_to_replace, rooms, clean_history_v1, get_current_room, clean_obj_name, focus_on_count, rank_candidates_by_common_words, gpt_select_valid
...
def score_based_search(roberta_model, roberta_tokenizer, input_str, env):
...
def eval(args, task_num, logger):
    ...
    lm_model, tokenizer, sbert_model, embed, roberta_model, roberta_tokenizer= load_model(args, device)
    ...
    for variation in variations:
        ...
        while not done:           
            ...
            if action is None and len(action_buffer) > 0:
                ...
                for action_ind, action_candidate in enumerate(action_buffer):
                    ...
                    for act_cand in action_trials:
                        ...
                        clean_recent_actions, clean_recent_obs, clean_recent_scores, clean_recent_reward, _, _ = \
                            clean_history_v1(recent_actions, recent_obs, recent_scores, recent_reward, recent_locs,
                                             recent_dis)
                        input_str, _ = compose_instance(mode=mode, step_id=step + 1, task_desc=task_description,
                                                        returns_to_go=returns_to_go,
                                                        curr_action=None, curr_obs=obs, inventory=info['inv'],
                                                        look=info['look'],
                                                        prev_action=prev_action, prev_obs=prev_obs, objects=objects,
                                                        places=places,
                        ...
                        extra_input_str = extracted_content(input_str)
                    ...
                    if action_accepted:
                        ...
                        action = final_action
                        buffer_overall_trail = 0
                        if args['roberta'] == True:
                            _, jug = get_model_roberta(args, extra_input_str, tokenizer, lm_model, device, logger, embed,
                    ...
                    if not action_candidate.startswith("focus on"):
                        ...
                        logger.info(f"top candidates are = [{candidates[:5]}] with gpt")
                        if len(candidates) == 0: 
                            failed_action_trial[action_candidate] += 1
                            ...
                            selections = gpt_select_valid(action_candidate, candidates, clean_look(info['look']), info['inv'], obs_buffer[action_ind], logger.info, 1, gpt_version, llm = args["local_llm"])
                            # # intersection = set(sbert_results) & set(edit_results)
                            # if len(intersection) == 0:
                            #     continue 


                            match = re.search(r'Action:(.*?)(?:[.,]|$)', selections)

                            if match:
                                logger.info(f"matching results = {match.group(1)}")
                                ...
                                if elem in candidates:
                                    ...
                                    action = findValidActionNew(elem, env, info['look'], recent_actions, sbert_model, logger, test = candidates)
                                    logger.info(f"no valid actions for matching results, change it to {action}")
                            else:
                                logger.info(f"no matching, need checking")
            ...
            # Get valid actions at this point               
                ...
                used_sys2, return_result = findValidActionWithSystem2(env, task_num, task_description, info['look'],
                                                            ...
                                                            useful_focus_on, focus_on_done, gpt_version, llm_name=args["local_llm"], llm=None)  
                if not used_sys2:
                    ...
                    action, jug = get_model_detee(args, input_str, tokenizer, lm_model, device, logger, embed, action)
                    jug = (int)(jug * 10)
            ...
            if reward > 0 and action.startswith("focus on"):
                ...
                if len(useful_focus_on) == max(focus_on_count[str(task_num)], task_description.count("focus")):
                    focus_on_done = True 
            logger.info(f"socre = {score},  length of rewarc = {len(recent_reward)}, sum of reward = {sum(recent_reward[-30:])}")
            if score < 0 or (len(recent_reward)>=100 and sum(recent_reward[-30:])==0):
                ...
                if args["no_stop"]:
                    ...
                    score = 0
                logger.info("done for lack reward")
            last_score = score
...
def parse_args():
    ...
    parser.add_argument("--lm_path", default="/home/qhf/ubuntu/git_clone_collection/SwiftSage/checkpoint-5800")
    ...
    parser.add_argument("--output_path", default="logs/fast_ap_log")
    ...
    parser.add_argument("--gpt_version", default="NONE", type=str)  
    parser.add_argument("--local_llm", default='llama3', type=str)  
    parser.add_argument("--demo_file", default="data_utils/demos.json", type=str)
    parser.add_argument("--debug_var", type=int, default=93)
    parser.add_argument("--abl", action="store_true", default=True)
